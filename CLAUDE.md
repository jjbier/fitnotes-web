...
- **i18n (2026-07-16)**: `react-i18next` + `i18next` en web y mobile (`expo-localization` en mobile para detectar el idioma del dispositivo como estimación inicial). Diccionarios `es`/`en` en `packages/core/src/i18n/locales/` (namespaces `common`/`settings`), con test de paridad de claves (`i18n.test.ts`) para que ambos idiomas no diverjan. Idioma persistido como preferencia de usuario (`UserPreferences.language`, ver arriba) — en mobile, `_layout.tsx` llama a `i18n.changeLanguage()` al hidratar preferencias. **Solo Settings está migrada** (pantalla piloto); el resto de la app sigue con strings hardcodeados en español — migrar pantalla a pantalla añadiendo su namespace a `locales/es.ts`/`en.ts` en ambos idiomas a la vez (el test de paridad falla si no).
