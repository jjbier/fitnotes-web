...
### Sin fix de fondo (requieren dispositivo físico / son limitaciones de diseño aceptadas)
- **Sesión Supabase no sobrevive a `force-stop` en mobile**: ya tiene un intento de fix serio (`apps/mobile/lib/supabase.ts` — cola de escrituras serializada + patrón write-tmp-then-rename para evitar corrupción de `FileStorage`), pero el propio código documenta que el síntoma persiste pese a ello. Diagnosticarlo del todo requiere reproducirlo en un dispositivo Android físico (no disponible en este entorno) — no se ha intentado un fix adicional sin poder verificarlo.
